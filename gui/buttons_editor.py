from wx import Frame, Panel, TextCtrl, Button, StaticText, Font, StaticLine, Icon, CheckBox
from wx import EVT_BUTTON, FONTFAMILY_DEFAULT, FONTSTYLE_NORMAL, FONTWEIGHT_NORMAL, ID_ANY, LI_HORIZONTAL, EVT_CLOSE, EVT_CHECKBOX, DEFAULT_FRAME_STYLE, RESIZE_BORDER, MAXIMIZE_BOX

from modules.data import data, LOGO
from gui.controller import gui
from gui.help import Help

class ButtonEditor(Frame):
    def __init__(self, parent):
        super().__init__(parent=parent, title='Редактор кнопок', size=(400, 600), style=DEFAULT_FRAME_STYLE & ~(RESIZE_BORDER | MAXIMIZE_BOX))
        ...
        self.track_show_first = CheckBox(self.panel, pos=(5, 80), label='Кнопка 1')
        self.track_show_first.Bind(EVT_CHECKBOX, self.show_track_first_func)
        self.track_show_first.SetValue(button.show_first)

        self.track_show_second = CheckBox(self.panel, pos=(80, 80), label='Кнопка 2')
        self.track_show_second.Bind(EVT_CHECKBOX, self.show_track_second_func)        
        self.track_show_second.SetValue(button.show_second)

        self.track_label_first = TextCtrl(self.panel, pos=(5, 105), size=(280, 20))
        StaticText(self.panel, pos=(290, 107), label='Заголовок 1')
        self.track_label_first.SetValue(button.label_first)

        self.track_url_first = TextCtrl(self.panel, pos=(5, 130), size=(280, 20))
        StaticText(self.panel, pos=(290, 132), label='Ссылка 1')
        ...
        self.track_label_second = TextCtrl(self.panel, pos=(5, 155), size=(280, 20))
        StaticText(self.panel, pos=(290, 157), label='Заголовок 2')
        self.track_label_second.SetValue(button.label_second)

        self.track_url_second = TextCtrl(self.panel, pos=(5, 180), size=(280, 20))
        StaticText(self.panel, pos=(290, 182), label='Ссылка 2')
        ...
        # Повтор
        StaticLine(self.panel, ID_ANY, pos=(0, 205), size=(400, 1), style=LI_HORIZONTAL)

        StaticText(self.panel, label='Трек повторяется', pos=(10, 208)).SetFont(big)

        StaticLine(self.panel, ID_ANY, pos=(0, 235), size=(400, 1), style=LI_HORIZONTAL)

        button = data.repeat.button
        self.repeat_show_first = CheckBox(self.panel, pos=(5, 240), label='Кнопка 1')
        self.repeat_show_first.Bind(EVT_CHECKBOX, self.show_repeat_first_func)
        self.repeat_show_first.SetValue(button.show_first)

        self.repeat_show_second = CheckBox(self.panel, pos=(80, 240), label='Кнопка 2')
        self.repeat_show_second.Bind(EVT_CHECKBOX, self.show_repeat_second_func)        
        self.repeat_show_second.SetValue(button.show_second)

        self.repeat_label_first = TextCtrl(self.panel, pos=(5, 265), size=(280, 20))
        StaticText(self.panel, pos=(290, 267), label='Заголовок 1')
        self.repeat_label_first.SetValue(button.label_first)

        self.repeat_url_first = TextCtrl(self.panel, pos=(5, 290), size=(280, 20))
        StaticText(self.panel, pos=(290, 295), label='Ссылка 1')
        ...
        self.repeat_label_second = TextCtrl(self.panel, pos=(5, 315), size=(280, 20))
        StaticText(self.panel, pos=(290, 317), label='Заголовок 2')
        self.repeat_label_second.SetValue(button.label_second)

        self.repeat_url_second = TextCtrl(self.panel, pos=(5, 340), size=(280, 20))
        StaticText(self.panel, pos=(290, 342), label='Ссылка 2')
        ...
        # Поток
        StaticLine(self.panel, ID_ANY, pos=(0, 365), size=(400, 1), style=LI_HORIZONTAL)

        StaticText(self.panel, label='Поток', pos=(10, 368)).SetFont(big)

        StaticLine(self.panel, ID_ANY, pos=(0, 395), size=(400, 1), style=LI_HORIZONTAL)

        button = data.wave.button
        self.wave_show_first = CheckBox(self.panel, pos=(5, 400), label='Кнопка 1')
        self.wave_show_first.Bind(EVT_CHECKBOX, self.show_wave_first_func)
        self.wave_show_first.SetValue(button.show_first)

        self.wave_show_second = CheckBox(self.panel, pos=(80, 400), label='Кнопка 2')
        self.wave_show_second.Bind(EVT_CHECKBOX, self.show_wave_second_func)        
        self.wave_show_second.SetValue(button.show_second)

        self.wave_label_first = TextCtrl(self.panel, pos=(5, 425), size=(280, 20))
        StaticText(self.panel, pos=(290, 427), label='Заголовок 1')
        self.wave_label_first.SetValue(button.label_first)

        self.wave_url_first = TextCtrl(self.panel, pos=(5, 450), size=(280, 20))
        StaticText(self.panel, pos=(290, 452), label='Ссылка 1')
        ...
        self.wave_label_second = TextCtrl(self.panel, pos=(5, 475), size=(280, 20))
        StaticText(self.panel, pos=(290, 477), label='Заголовок 2')
        self.wave_label_second.SetValue(button.label_second)

        self.wave_url_second = TextCtrl(self.panel, pos=(5, 500), size=(280, 20))
        StaticText(self.panel, pos=(290, 502), label='Ссылка 2')
