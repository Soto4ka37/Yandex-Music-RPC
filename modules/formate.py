...
def formate_string(string: str) -> str | None:
    def edit(string: str, replace: str = None, to: str = None):
        if replace and string and replace in string:
            return string.replace(replace, str(to))
        return string

    string = edit(string, '%ver%', str(VERSION)) # Версия программы   
    string = edit(string, '%queue-num%', str(api.queue_index)) # Номер текущего трека в очереди
    string = edit(string, '%queue-count%', str(api.queue_len)) # Число треков в очереди
    string = edit(string, '%description%', str(api.queue_index)) # Название потока
    if api.track:
        track = api.track
        album = track.album
        ...
        string = edit(string, '%album-count%', str(album.count)) # Количество треков в альбоме
